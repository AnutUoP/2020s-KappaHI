...
F3 = np.zeros((5,5))
for i in range(16):
   F = F+np.loadtxt('F_ij_p18vHIHI_zH'+str(i+1)+'_bias1_noise.mat')
   if(i==3):
      F1=F1+F 
   if(i==7):
      F2=F2+F
...
g.triangle_plot([gauss2,gauss3,gauss4,gauss1],filled=True,legend_labels=['4-HIHI','8-HIHI','12-HIHI','16-HIHI'])
...
plt.savefig('bias1_planck_16HIHI_noise.png')
plt.close()
...
plt.legend()
#plt.show()
plt.savefig('FoM_HIHI_noise.png')