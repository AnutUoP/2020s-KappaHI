...
for i in range(5):
    u[i] = hp.read_cl('Planck_'+str(i)+'_HI.23.fits') 
C6_9 = np.zeros((15,15))
C9_9 = np.zeros((15,15))
C13_9 = np.zeros((15,15))
Ck9 = np.zeros((15,15))

C6_9 += np.loadtxt('../matrix/HI0.23_0.23_cov_av.mat') # 15 bins 35 realisation 
C9_9 += np.loadtxt('../matrix/HI0.23_0.35_cov_av.mat')
C13_9 += np.loadtxt('../matrix/HI0.23_0.41_cov_av.mat')
...
for i in range (5):
    for j in range (5):
        F_HI6_9[i][j] = np.matmul(u_HI6_9[i].transpose(),np.matmul(C6_9_inv,u_HI6_9[j]))[0][0]
...
F = F_HI6_9 + F_HI9_9 + F_HI13_9 + F_k9

#print F

print (np.matmul(F_HI6_9,inv(F_HI6_9)))
...
np.savetxt('F_ij_p18vHI.23_bias1.mat',F)

np.savetxt('F_p18vHI.23_bias1_inv.mat',inv(F))
